...
class ScanNetpp_Seq(BaseStereoViewDataset):
    def __init__(self,  
        ...
        # For inference/testing, try to use a default validation list if available; optional scene_name overrides.
        list_file = 'nvs_sem_val.txt'
        split_path = os.path.join(ROOT, 'splits', list_file)
        if os.path.exists(split_path):
            with open(split_path, 'r') as f:
                self.scene_names = f.read().splitlines()
        else:
            # Fallback: infer scene folders from ROOT
            try:
                self.scene_names = [d for d in os.listdir(self.ROOT) if os.path.isdir(os.path.join(self.ROOT, d))]
            except Exception:
                self.scene_names = []
